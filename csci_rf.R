...
# Examine the environment to be sure # of observations looks like the 75/25 split. 1516:5036.
...
ca_predictions2 <- ca_predictions2 %>%
  mutate(classification = case_when(csci_predicted < 0.61~"Very Likely Altered",
    csci_predicted < 0.79~"Likely Altered",
    csci_predicted < 0.92~"Possibly Altered",
    csci_predicted >= 0.92~"Likely Unaltered")) %>%
...
csci_lms <- read_csv("csci_lms.csv")
